    ...
    useEffect(() => {
        if (branch && isOpen) {
            reset({
                ...
                maxPartySize: branch.bookingConfig?.maxPartySize || branch.maxPartySize || 50,
                bookingWindowDays: branch.bookingConfig?.bookingWindowDays || branch.bookingWindowDays || 7,
                cancellationHours: branch.bookingConfig?.cancellationHours || branch.cancellationHours || 24,
                depositRequired: branch.bookingConfig?.depositRequired || branch.depositRequired || false,
                depositPerPax: branch.bookingConfig?.depositAmount || branch.depositPerPax || 0,
                isVatInclusive: branch.taxConfig?.isVatInclusive || branch.isVatInclusive || false,
                defaultVatRate: branch.taxConfig?.defaultVatRate || branch.defaultVatRate || 10,
                applyServiceCharge: branch.taxConfig?.applyServiceCharge || branch.applyServiceCharge || false,
                serviceChargeRate: branch.taxConfig?.serviceChargeRate || branch.serviceChargeRate || 0,
                logo: branch.logo || "",
