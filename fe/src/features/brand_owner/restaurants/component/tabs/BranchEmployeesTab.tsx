...
import { useGetStaffs } from "@/src/features/brand_owner/staffs/hook/useGetStaffs";
...
import CreateStaffForm from "@/src/features/brand_owner/staffs/component/CreateStaffForm";
import UpdateStaffForm from "@/src/features/brand_owner/staffs/component/UpdateStaffForm";
import { ConfirmModal } from "@/src/core/components/layout/public-ConfirmModal";
import { useDeleteStaff } from "@/src/features/brand_owner/staffs/hook/useDeleteStaff";

interface BranchEmployeesTabProps {
    id_brand: string;
    restaurantId: string;
}

const BranchEmployeesTab: React.FC<BranchEmployeesTabProps> = ({ id_brand, restaurantId }) => {
    ...
    const { mutateAsync: deleteStaff, isPending: isDeleting } = useDeleteStaff(id_brand);
    ...
    return (
        <FadeIn className="w-full flex flex-col gap-6">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <div>
                    <h3 className="text-xl font-bold text-gray-800">Quản lý nhân sự chi nhánh</h3>
                    <p className="text-sm text-gray-500 mt-1">Quản lý tài khoản và phân quyền cho nhân viên làm việc tại đây.</p>
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col w-full">
                {isLoading ? (
                    ...
                    <div className="flex justify-center items-center py-20 text-red-500">Có lỗi xảy ra khi lấy danh sách.</div>
                ) : !data || data.items.length === 0 ? (
                    <div className="flex justify-center items-center py-20 text-gray-500">Chi nhánh này chưa có nhân viên nào.</div>
                ) : (
                    <>
...
export default BranchEmployeesTab;
