...
import { upsertOperatingHoursSchema, UpsertOperatingHoursFormValues } from "@/src/features/brand_owner/operating_hours/schema/operating_hours.upsert.schema";
import { useGetManagerOperatingHours } from "../hook/useGetManagerOperatingHours";
import { useUpsertManagerOperatingHours } from "../hook/useUpsertManagerOperatingHours";
...
const ManagerOperatingHoursTab: React.FC = () => {
    const activeWorkspace = useAuthStore((state) => state.activeWorkspace);
    const restaurantId = activeWorkspace?.id || "";

    const { data: operatingHours, isLoading } = useGetManagerOperatingHours(restaurantId);
    const { mutate: upsert, isPending } = useUpsertManagerOperatingHours(restaurantId);
    ...
    useEffect(() => {
        if (operatingHours && operatingHours.length === 7) {
            form.reset({
                operating_hours: [...operatingHours].sort((a: any, b: any) => a.day_of_week - b.day_of_week).map((item: any) => ({
    ...
    const onSubmit = (values: UpsertOperatingHoursFormValues) => {
        upsert(values.operating_hours as any);
    };
                ...
                <div className="w-full flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
                    <div>
                        <H className="text-xl font-bold text-gray-800 flex items-center gap-2">
                            ...
                            Thời gian hoạt động (Chi nhánh)
                        </H>
                        <P className="text-sm text-gray-500 mt-1">
                            Thiết lập giờ mở cửa, đóng cửa và giờ nghỉ trưa cho chi nhánh của bạn.
                        </P>
                    </div>
                    <Button 
                        variant="default" 
...
export default ManagerOperatingHoursTab;
