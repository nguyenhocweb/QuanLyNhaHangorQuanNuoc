...
import { useUpdateApiKey, useGetActiveAiModels } from '../hook/useApiKeys';
...
export const SystemUpdateApiKeyModal: React.FC<Props> = ({ isOpen, onClose, apiKey }) => {
  ...
  return (
    <Modal open={isOpen} onClose={handleClose} title="Cập nhật API Key (Global)">
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col w-full gap-5">
        <P className="text-gray-500 text-sm">
          Bạn có thể thay đổi tên và giới hạn Model cho API Key này. Quản trị viên hệ thống có thể cập nhật key này mà không bị giới hạn.
        </P>
        ...
        <div className="flex flex-col w-full gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-gray-700">Phân quyền phạm vi sử dụng <span className="text-red-500">*</span></label>
            <Controller
              name="keyType"
              control={control}
              render={({ field }) => (
                <select
                  ...
                  className="rounded-xl border-gray-300 px-4 py-2.5 bg-gray-50 border focus:ring-indigo-500 focus:border-indigo-500 text-sm w-full outline-none transition-all disabled:opacity-50"
                >
                  <option value="BRAND">Dành cho Thương Hiệu (Brand)</option>
                  <option value="CUSTOMER">Dành cho Khách Hàng (Customer)</option>
                  <option value="ADMIN">Dành riêng cho Admin (Hệ thống)</option>
                </select>
              )}
            />
            {errors.keyType && <span className="text-xs text-red-500">{errors.keyType.message}</span>}
          </div>
