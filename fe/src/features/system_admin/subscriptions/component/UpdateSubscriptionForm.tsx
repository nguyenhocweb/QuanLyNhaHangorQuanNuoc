...
import { useUpdateSubscription, useGetSubscriptionFeatures } from '../hook/useSubscription_hook';
...
export const UpdateSubscriptionForm: React.FC<UpdateSubscriptionFormProps> = ({ initialData, onClose, onSuccess }) => {
    const { register, handleSubmit, setValue, watch, formState: { errors }, reset } = useForm<SubscriptionFormValues>({
        ...
        defaultValues: {
            ...
            featuresData: initialData.featuresData || {},
    ...
    useEffect(() => {
        reset({
            ...
            discountStartDate: initialData.discountStartDate ? new Date(initialData.discountStartDate).toISOString().split('T')[0] as any : undefined,
            discountEndDate: initialData.discountEndDate ? new Date(initialData.discountEndDate).toISOString().split('T')[0] as any : undefined,
            billingCycle: initialData.billingCycle,
            maxRestaurants: initialData.maxRestaurants,
            trialPeriodDays: initialData.trialPeriodDays || 0,
            setupFee: initialData.setupFee || 0,
    ...
    const onSubmit = (data: SubscriptionFormValues) => {
        updateSubscription({ id: initialData.id, ...data }, {
            onSuccess: () => {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 overflow-y-auto p-4 sm:p-10">
            <div className="bg-white flex flex-col relative p-6 sm:p-8 rounded-2xl shadow-2xl w-full max-w-[800px] border border-gray-100 my-auto">
                ...
                <div className="mb-8 border-b border-gray-100 pb-4">
                    <h2 className="text-2xl font-bold text-gray-900">Cập nhật Gói Cước</h2>
                    <p className="text-gray-500 text-sm mt-1.5">Chỉnh sửa thông tin gói cước ID: {initialData.id}</p>
                </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div className="flex justify-end gap-3 pt-6 border-t border-gray-100">
                        ...
                        <button disabled={isPending} type="submit" className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-[14px] font-medium rounded-xl transition-colors flex items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed">
                            {isPending ? 'Đang cập nhật...' : <><FiCheck className="text-lg" /> Cập nhật gói cước</>}
                        </button>
                    </div>
                </form>
            </div>
